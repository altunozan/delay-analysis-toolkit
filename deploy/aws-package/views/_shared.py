...
# The deployment's managed AI credential: GEMINI ONLY in this build.
# Set GEMINI_API_KEY (or GOOGLE_API_KEY) in the deployment's secrets
# and every AI feature runs zero-click on it, the key held server-side
# and never rendered.
_MANAGED_SOURCES = (
...
def managed_ai_credentials() -> tuple[str, str]:
    """(provider, key) of the deployment's managed AI credential.

    Resolution: Streamlit secrets, then environment, in
    _MANAGED_SOURCES order. The key is NEVER written into the
    repository and NEVER rendered — callers only ever learn whether one
    exists, not what it is. Absent = the app simply asks the analyst
    for their own key, exactly as before.
    """
    for provider, names in _MANAGED_SOURCES:
        for name in names:
            try:
                v = st.secrets.get(name, "")
            except Exception:              # no secrets.toml present
                v = ""
            v = (v or os.environ.get(name, "")).strip()
            if v:
                return provider, v
    return "", ""
...
def ai_credentials_panel(page: str) -> None:
    ...
    Default path: the deployment's managed Gemini key runs everything
    with no setup and is never displayed. The analyst may instead
    supply their own key for Anthropic / OpenAI / Gemini, which takes
    precedence for the rest of the session. Widgets are page-local (widget-backed state dies
    when its page is not rendered); values are copied into the plain
    shared keys so the choice survives navigation.
    """
    m_prov, managed = managed_ai_credentials()

    if managed and st.session_state.get(sk.AI_MANAGED, True):
        # ---- managed default: no key input rendered at all ----------
        st.session_state[sk.AI_PROVIDER] = m_prov
        st.session_state[sk.AI_KEY] = managed
        st.session_state[sk.AI_MANAGED] = True
        pinfo = PROVIDERS[m_prov]
        _mlabel = pinfo["label"].split(" (")[0]
        c1, c2 = st.columns([1, 1])
        c1.success(f"AI enabled — managed {_mlabel} endpoint. "
                   "No key needed.")
        st.session_state[sk.AI_MODEL] = model_selector(
            c2, pinfo, f"aic_model_{page}_{m_prov}")
        st.caption(
            f"Narratives run on a managed {_mlabel} endpoint provided "
            "with this deployment; the credential is held server-side "
            "and is not shown or exported. Prompts carry the figures "
            "and activity names of the programmes you load — if the "
            "matter forbids third-party processing, switch to your own "
            "key or a self-hosted endpoint below.")
    ...
    if managed:
        st.caption("Using your own key. The managed endpoint "
                   "remains available.")
    a1, a2 = st.columns(2)
...
def resolve_ai_credentials() -> tuple[str, str, str]:
    ...
    if managed and st.session_state.get(sk.AI_MANAGED, True):
        ...
        if model not in pinfo.get("models", [pinfo["default_model"]]):
            model = pinfo["default_model"]  # never a cross-provider model
        return (m_prov, model, managed)
    return (st.session_state.get(sk.AI_PROVIDER, m_prov or "gemini"),
...
def ai_provider_block(state_key: str) -> tuple[str, str | None, str]:
    ...
    _m_prov, _managed = managed_ai_credentials()
    ...
    if _use_managed:
        ...
        _mlabel = PROVIDERS[_m_prov]["label"].split(" (")[0]
        pcol1.caption(f"Managed {_mlabel} endpoint — no key required.")
        provider = _m_prov
        ...
        if _managed and st.button("Back to the managed endpoint",
                                  key=f"{state_key}_bk"):
            st.session_state[sk.AI_MANAGED] = True
            st.rerun()
